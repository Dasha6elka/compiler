...
export namespace lexer {
    export function main(inputFile: string, tokensFile: string, result: string[]): string[] {
        lineReader.open(inputFile, function (_, reader) {
            let lineCount = 1;
            let resultString = "";

            while (reader.hasNextLine()) {
                reader.nextLine(function (_, line) {
                    let output = "";
                    const str = lineCount;
                    let i = 0;
                    ...
                    while (i < line.length) {
                                    ...
                                    while (nextVal != '"') {
                                        ...
                                        if (i == line.length) {
                                            if (reader.hasNextLine()) {
                                                reader.nextLine(function (_, nextLine) {
                                                    line = nextLine;
                                                });
                                            }
                                            i = 0;
                        ...
                        if (
                            ...
                            resultString += currToken.toString() + " " + output + " " + str + " " + position + "\n";
            ...
            fs.writeFileSync(tokensFile, resultString);
        });
